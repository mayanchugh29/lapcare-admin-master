...
const CreateShipment = (props) => {
  ...
  const [payment_method, setpayment_method] = useState(orderData.paymentMethod)
  const [packageWeight, setpackageWeight] = useState(0.5)
  const [length, setlength] = useState()
  const [width, setwidth] = useState()
  const [height, setheight] = useState()
  const [orderValue, setorderValue] = useState(orderData.totalPrice)
  ...
  const handleShippingOnClickEvent = async () => {
    ...
    const response = await request("/shipping/create", data, props.token)
    if (response.status === 200) {
      dispatch({
        type: SET_TOASTIFY,
        payload: {
      ...
      setdisableShipmentButton(false)
    } else {
      dispatch({
        type: SET_TOASTIFY,
        payload: {
  ...
  const handleChange = (event) => {
    setpickup_pincode(event.target.value.pin_code)
    setpickupLocation(event.target.value.pickup_location)
  }
        ...
        <CardContent>
          ...
          <div className={classes.div}>
            <FormControl
              ...
              <InputLabel id="demo-simple-select-label">Pickup From</InputLabel>
              <Select
                ...
                {avilablePincodes.map((availablePincode) => (
                  <MenuItem value={availablePincode}>
            ...
            <TextField
              ...
              onChange={(event) => setdelivery_pincode(event.target.value)}
              spellCheck="false"
          ...
          {showCouriers ? (
            <div className={classes.parentBox}>
              ...
              <div
                ...
                {selectedCourierName ? (
                  <Button
                    disabled={disableShipmentButton}
                    variant="contained"
                    color="primary"
...
export default connect(mapStateToProps)(withRouter(CreateShipment))
