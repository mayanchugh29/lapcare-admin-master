...
const EditCategory = (props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const response = await request("/category/edit", values, props.token)
      if (response.status === 200) {
        ...
        dispatch({
          ...
          payload: {
            open: true,
            msg: "Product Category could not be edited!",
            type: "error",
          },
        })
      }
...
export default connect(mapStateToProps)(withRouter(EditCategory))
